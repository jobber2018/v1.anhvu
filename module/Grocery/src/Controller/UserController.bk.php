...
//        var_dump($groceryCatData);
    ...
    public function listAction()
    {
        ...
        return new ViewModel(["groceryList"=>$groceryListByCat,"groceryCat"=>$groceryCat,'textSearch'=>$textSearch]);
    }
        ...
        if($request->isPost()){
            ...
            if($form->isValid()){
                ...
                try{
                    ...
                    if($groceryViaMobile){
                        $msgErr='';
                        foreach ($groceryViaMobile as $groceryItem)
                            $msgErr='Số điện thoại <b>'.$groceryItem->getMobile() .'</b> của chủ cửa hàng <b>'. $groceryItem->getGroceryName().'</b> đã có trong hệ thống!';
                            throw new Exception($msgErr);
//                        throw new Exception('Số điện thoại <b>'.$mobile .'</b> của chủ cửa hàng <b>'. $groceryViaMobile->getGroceryName().'</b> đã có trong hệ thống!');
                    }
    ...
    public function mapPositionAction(){
        ...
        if($request->isPost()){
            ...
            if($form->isValid()){
                ...
                $this->flashMessenger()->addSuccessMessage('Thêm mới thành công '. $grocery->getGroceryName());
...
//            return new JsonModel($result);
            return $this->redirect()->toRoute('grocery-user',['action'=>'panel','id'=>$grocery->getId()]);
        }
    }
...
//        $userId= $this->userInfo->getId();
    ...
    public function detailAction(){
        ...
        return new ViewModel(["groceryDetail"=>$groceryDetail]);
    }
